...
[NOTE]
====
In the example above we allow access to both the named `webhook-server` port in the Operator `Pod` and to the https
port 443 which is exposed by the web-hook `Service`.
====
