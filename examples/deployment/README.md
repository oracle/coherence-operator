...
You can use `helm create` for any of the examples to install that one directly.

# Table of Contents

* [Prerequisites](#prerequisites)
  * [Coherence Operator Quick Start](#coherence-operator-quick-start) 
  * [JDK and Maven Vvrsions](#jdk-and-maven-versions) 
...
Ensure you have followed all the [Quick Start Guide](https://oracle.github.io/coherence-operator/docs-unstable/2.0.0-1910091023/#/about/03_quickstart) including the
...
```bash
kubectl create namespace coherence-example-ns

namespace/coherence-example-ns created
```

> Note: In the examples, a Kubernetes namespace called `coherence-example-ns` is used. 
...
the secret named `coherence-example-secret` is used in the namespace `coherence-example-ns`.

```bash
kubectl -n coherence-example-ns \
...
The examples exist in the `2.0` branch of the Coherence Operator GitHub repository - https://github.com/oracle/coherence-operator.
...
Ensure you have Docker running and your Mavent and JDK11 build environemnt set and use the 
...
> Note: The following uses the `coherence-unstable` helm repository as well as sets a specific pre-release version.
...
--set installEFK=true \
--namespace coherence-example-ns \
...
```bash
kubectl get pods -n coherence-example-ns
...
*   Port-forward Grafana for viewing metrics
    ...
    ```bash
    ./port-forward-grafana.sh coherence-example-ns   

    Port-forwarding coherence-operator-grafana-8454698bcf-5dqvm in coherence-example-ns
...
*   Port-forward Kibana for viewing log messages
    ...
    ```bash
    ./port-forward-kibana.sh coherence-example-ns       
  
    Port-forwarding kibana-67c4f74ffb-nspwz in coherence-example-ns
...
1.  Install the Coherence cluster `storage` role
    ...
    ```bash   
    kubectl -n coherence-example-ns create -f src/main/yaml/example-cluster.yaml 
    ```       

    List the created `coherenceclusters` and `coherenceroles` using:

    ```bash
    kubectl get coherenceclusters -n coherence-example-ns
   
    NAME                AGE
    example-cluster     77s         

    kubectl get coherenceroles -n coherence-example-ns
   
    NAME                      AGE
    example-cluster-storage   79s
    ```           
...
1.  View the running pods
    ...
    > Note: You may also use `kubectl get pods -n coherence-example-ns` to view all pods including those  
...
1.  Connect to the Coherence Console inside the cluster to add data
    ...
    ```bash
    kubectl exec -it --namespace coherence-example-ns example-cluster-storage-0 bash /scripts/startCoherence.sh console
    ```
...
1.  Scale the `storage` role to 6 members
    ...
    ```bash
    kubectl -n coherence-example-ns scale coherencerole/example-cluster-storage --replicas=6
    ```
...
1.  Scale the `storage` role back to 3 members
    ...
    ```bash
    kubectl -n coherence-example-ns scale coherencerole/example-cluster-storage --replicas=3
    ```                               
...
The extra yaml below has been added which shows:
...
1.  Install the `proxy` role
    ...
    ```bash
    kubectl -n coherence-example-ns apply -f src/main/yaml/example-cluster-proxy.yaml 
...
1.  Port forward the proxy port
    ...
    ```bash
    kubectl port-forward -n coherence-example-ns example-cluster-proxy-0 20000:20000
    ``` 
...
adds a new role `helidon-app`. This role defines a user application which uses [Helidon](https://helidon.io/) to create a
`/query` endpoint allowing the user to send CohQL commands via this endpoint.
 The extra yaml below has been added which shows:

* A port called `helidon` being exposed on 8080 for the application
* The role being set as storage-disabled
...
1.  Install the `helidon-app' role
    ...
    ```bash
    kubectl -n coherence-example-ns apply -f src/main/yaml/example-cluster-app.yaml  
    
    kubectl get coherenceroles -n coherence-example-ns
    NAME                          AGE
    example-cluster-helidon-app   11s
...
1.  View the running pods
    ...
    NAME                            READY   STATUS    RESTARTS   AGE
    example-cluster-helidon-app-0   1/1     Running   0          5s
...
1.  Port forward the application port
    ...
    ```bash
    kubectl port-forward -n coherence-example-ns example-cluster-helidon-app-0 8080:8080
    ``` 

1.  Access the custom `/query` endpoint

    Use the various `CohQL` commands to access, and mutate data in the Coherence cluster.
...
The snippet of yaml below shows:
...
1.  Delete the existing deployment
    ...
    ```bash
    kubectl -n coherence-example-ns delete -f src/main/yaml/example-cluster-app.yaml
    ```                                   
...
1.  Check Active Persistence is enabled
    ...
    ```bash
    kubectl logs example-cluster-storage-0 -n coherence-example-ns | grep 'Created persistent'
...
1.  Delete the cluster
    ...
    Use `kubectl get pods -n coherence-example-ns` to confirm the pods have terminated.
...
1.  Delete the PVC's
    ...
    ```bash
    kubectl get pvc -n coherence-example-ns | sed 1d | awk '{print $1}' | xargs kubectl delete pvc -n coherence-example-ns
    ```
