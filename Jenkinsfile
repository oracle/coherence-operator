    ...
    stages {
        ...
        stage('docker-build') {
		    ...
		    env
                '''
                withMaven(jdk: 'Jdk11', maven: 'Maven3.6.0', mavenSettingsConfig: 'coherence-operator-maven-settings', tempBinDir: '') {
                    sh 'cd docs/samples && mvn -Dhttp.proxyHost=$http_proxy_host -Dhttps.proxyHost=$https_proxy_host -Dhttp.proxyPort=$http_proxy_port -Dhttps.proxyPort=$https_proxy_port -Pdocker,docker-v1,docker-v2 clean install '
                }
            }
        }
        stage('docker-push') {
