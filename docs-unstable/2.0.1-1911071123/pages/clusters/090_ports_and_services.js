...
<li data-value="3">Optionally use a different port in the <code>Service</code> to that used by the <code>Container</code>. If the <code>service.port</code> is not
specified the same value will be used for both the container port and service port.</li>
</ul>
...
will still be added as a named port to the Coherence <code>Container</code> spec in the <code>Pod</code>. An example of when service creation
needs to be disabled is when the <code>Service</code> will be created externally and not managed by the operator.</p>
...
<h3 id="_adding_service_annotations">Adding Service Annotations</h3>
<div class="section">
<p>It is possible to add extra annotations to the <code>Service</code> created for a port by adding additional annotations to the
<code>service.annotations</code> field. The format of the annotations is exactly the same as when creating a
<a id="" title="" target="_blank" href="https://kubernetes.io/docs/concepts/services-networking/service/">Kubernetes Service</a> as also documented in
the <a id="" title="" target="_blank" href="https://kubernetes.io/docs/concepts/overview/working-with-objects/annotations/">Kubernetes annotations</a>
documentation.</p>
...
<p>The <code>http</code> port created from the yaml above will be exposed on a service that looks like the following:</p>
...
spec:
  ports:
    - name: http
      port: 8080
      targetPort: 8080</markup>

<ul class="colist">
<li data-value="1">The additional annotations from the <code>http</code> port&#8217;s <code>service</code> configuration have been added to the <code>Service</code></li>
</ul>
</div>

<h3 id="_adding_service_labels">Adding Service Labels</h3>
<div class="section">
<p>It is possible to add extra labels to the <code>Service</code> created for a port by adding additional labels to the
<code>service.labels</code> field. The format of the labels is exactly the same as when creating a
<a id="" title="" target="_blank" href="https://kubernetes.io/docs/concepts/services-networking/service/">Kubernetes Service</a> as also documented in
the <a id="" title="" target="_blank" href="https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/">Kubernetes labels</a> documentation.</p>
...
<ul class="colist">
<li data-value="1">The additional labels from the <code>http</code> port&#8217;s <code>service</code> configuration have been added to the <code>Service</code></li>
</ul>
</div>
</div>
</doc-view>
