...
If the <code>jvm.loggingConfig</code> field is set the value is used to set the <code>java.util.logging.config.file</code>
system property.
If <code>jvm.loggingConfig</code> is set the operator will also set the <code>coherence.log</code> system property to <code>jdk</code>
(i.e. <code>-Dcoherence.log=jdk</code>), this tells Coherence to use the JDK logger (i.e. Java util logging).
...
spec:
  jvm:
    loggingConfig: /coherence-operator/utils/logging/logging.properties</markup>

<p>The <code>jvm.loggingConfig</code> field has been set to use the config file provided by the operator.</p>
...
spec:
  jvm:
    loggingConfig: /coherence-operator/utils/logging/logging.properties
  volumes:
...
spec:
  ...
  configMapVolumes:
    - name: "efk-config"                                                               <span class="conum" data-value="3" />
...
<li data-value="3">An additional volume has been added from a <code>ConfigMap</code> named <code>efk-config</code>, that contains the Fluentd configuration to use.
