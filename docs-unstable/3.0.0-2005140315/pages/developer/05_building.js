...
this suffix is <code>ci</code> so the default version of the build artifacts is <code>3.0.0-ci</code>. Change this suffix, for
...
<div class="section">
<p>The Coherence Operator contains tests that can be executed using <code>make test</code>.</p>
...
<p>It is possible to run a sub-set of the tests or an individual test by using the <code>GO_TEST_FLAGS_E2E=&lt;regex&gt;</code> parameter.
...
>make e2e-local-test GO_TEST_FLAGS_E2E='-run=^TestMinimalCoherenceCluster$$'</markup>
...
>make e2e-local-test  GO_TEST_FLAGS_E2E='-run=^TestOneRole.*'</markup>

<p><strong>Note</strong> Any <code>$</code> signs in the reg-ex need to be escaped by using a double dollar sign <code>$$</code>.</p>

<p>The <code>GO_TEST_FLAGS_E2E</code> parameter can actually consist of any valid argument to be passed to the <code>go test</code> command.
There is plenty of documentation on <a id="" title="" target="_blank" href="https://tip.golang.org/cmd/go/#hdr-Test_packages">go test</a></p>
...
<p>As with local tests the <code>GO_TEST_FLAGS_E2E</code> parameter can be used to execute a sub-set of tests or a single test.</p>
