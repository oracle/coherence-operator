...
<h3 id="_explicit_roles_shared_values">Explicit Roles - Shared Values</h3>
<div class="section">
<p>When defining explicit roles in the <code>roles</code> list any values added to the <code>CoherenceCluster</code> <code>spec</code> directly (where an
implicit role would normally be configured) become default values shared by all roles in the <code>roles</code> list unless specifically
overridden by a <code>role</code>. This makes it easier to maintain configuration common to all roles in a single location in the
spec.</p>
...
spec:
  ...
  roles:
    - role: data   <span class="conum" data-value="1" />
    - role: proxy  <span class="conum" data-value="2" /></markup>

<ul class="colist">
<li data-value="1">The roles list contains two explicit roles, <code>data</code></li>
<li data-value="2">and <code>proxy</code></li>
<li data-value="3">The <code>replicas</code> value is set at the <code>spec</code> level and so will be shared by both of the explicit roles</li>
</ul>
<p>In the above example the cluster will have a total of 12 members, six for each role.</p>
...
spec:
  ...
  roles:
    - role: data
    - role: proxy
    - role: web    <span class="conum" data-value="1" />
      replicas: 2  <span class="conum" data-value="3" /></markup>

<ul class="colist">
<li data-value="1">Now a new <code>web</code> role has been introduced</li>
<li data-value="2">The replicas count in the <code>spec</code> section will still apply to the <code>data</code> and <code>proxy</code> roles, whch will each have a
replica count of 6</li>
<li data-value="3">The <code>web</code> role has a specific <code>replicas</code> value which will override the <code>spec.replicas</code> value so the <code>web</code> role will
have two replicas</li>
</ul>
</div>
</div>
</doc-view>
