...
<div class="section">
<p>The Coherence POF configuration file for a role in a <code>CoherenceCluster</code> is set using the <code>pofConfig</code> field of a role spec.
The value of this field will end up being passed to the Coherence JVM as the <code>coherence.pof.config</code> system property and
will hence set the value of the POF configuration file used as described in the Coherence documentation.</p>


<h3 id="_set_the_cache_configuration_for_an_implicit_role_2">Set the Cache Configuration for an Implicit Role</h3>
<div class="section">
<p>When using the implicit role configuration the <code>pofConfig</code> value is set directly in the <code>CoherenceCluster</code> <code>spec</code> section.
For example:</p>
...
metadata:
  name: test-cluster
spec:
  pofConfig: application-pof-config.xml  <span class="conum" data-value="1" /></markup>

<ul class="colist">
<li data-value="1">In this case a cluster will be created with a single implicit role named <code>storage</code> where the <code>coherence.pof.config</code>
system property and hence the POF configuration file used will be <code>application-pof-config.xml</code></li>
</ul>
</div>

<h3 id="_set_the_cache_configuration_for_explicit_role_2">Set the Cache Configuration for Explicit Role</h3>
<div class="section">
<p>When using the explicit role configuration the <code>pofConfig</code> value is set for each role in the <code>CoherenceCluster</code> <code>spec</code>
<code>roles</code> list.</p>
...
spec:
  roles:
    - role: data
      pofConfig: data-pof-config.xml  <span class="conum" data-value="1" />
    - role: proxy
      pofConfig: proxy-pof-config.xml  <span class="conum" data-value="2" /></markup>

<ul class="colist">
<li data-value="1">The <code>data</code> role will use the <code>data-pof-config.xml</code> POF configuration file</li>
<li data-value="2">The <code>proxy</code> role will use the <code>proxy-pof-config.xml</code> POF configuration file</li>
</ul>
</div>

<h3 id="_set_the_cache_configuration_for_explicit_roles_with_a_default_2">Set the Cache Configuration for Explicit Roles with a Default</h3>
<div class="section">
<p>When using the explicit role configuration a value for <code>pofConfig</code> value can be set in the <code>CoherenceCluster</code> <code>spec</code>
section that will be used as the default <code>pofConfig</code> value for any <code>role</code> in the <code>roles</code> list that does not explicitly
specify a value.</p>
...
metadata:
  name: test-cluster
spec:
  pofConfig: application-pof-config.xml  <span class="conum" data-value="1" />
  roles:
    - role: data
    - role: proxy
    - role: web
      pofConfig: web-pof-config.xml  <span class="conum" data-value="2" /></markup>

<ul class="colist">
<li data-value="1">The default <code>pofConfig</code> value is set to <code>application-pof-config.xml</code>. The <code>data</code> and <code>proxy</code> roles do not have
a <code>pofConfig</code> value so will use this default value and will each have use the <code>application-pof-config.xml</code> file</li>
<li data-value="2">The <code>web</code> role has an explicit <code>pofConfig</code> value of <code>web-pof-config.xml</code> so will use the <code>web-pof-config.xml</code>
POF configuration file</li>
</ul>
</div>
</div>
</doc-view>
