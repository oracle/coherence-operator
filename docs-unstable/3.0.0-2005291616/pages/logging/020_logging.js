...
<div class="section">
<div class="admonition note">
<p class="admonition-inline">Logging to files is the default setting for Coherence when running in containers started by the operator so in fact
...
different configuration be required.</p>
</div>

<h4 id="_coherence_jdk_logger">Coherence JDK Logger</h4>
<div class="section">
<p>The operator will set the <code>coherence.log</code> system property to <code>jdk</code> (i.e. <code>-Dcoherence.log=jdk</code>), this tells Coherence to
use the JDK logger (i.e. Java util logging).
See the Coherence <a id="" title="" target="_blank" href="https://docs.oracle.com/en/middleware/standalone/coherence/14.1.1.0/develop-applications/operational-configuration-elements.html#GUID-6116DB0E-91CC-4B4E-82B9-C6FE5E98BFBF">Logging Config</a>
documentation for more details on this setting.</p>

</div>

<h4 id="_default_logging_configuration_file">Default Logging Configuration File</h4>
<div class="section">
<p>In the <code>jvm</code> section of the <code>Coherence</code> CRD is a field to set the configuration file used by Java util logging.
see the <router-link to="/jvm_settings/060_logging">JVM Logging</router-link> section of the documentation.
If the <code>jvm.loggingConfig</code> field is not set a default configuration file, injected by the Operator, will be used.
This file configures Coherence to write log files to the <code>/logs</code> directory. The log files will have the name
<code>coherence-%g.log</code>, where <code>%g</code> is the log file generation created as logs get rotated.</p>

<p>The default Java util logging configuration will look something liek this:</p>
...
<div class="section">
<p>The logging configuration above configurs Coherence to write logs to the <code>/logs</code> directory.
The <code>Pods</code> created by the Operator have an empty-directory <code>Volume</code> named <code>logs</code> configured.
The <code>coherence</code> container has a volume mount configured to mount the <code>logs</code> <code>Volume</code> at the <code>/logs</code> mount path.</p>

<p>By using a <code>Volume</code>, this means that the <code>logs</code> volume is visible to all containers in the <code>Pod</code>, which is obviously
required to be able to access Coherence logs from a side-car container.</p>
...
spec:
  ...
  sideCars:
    - name: fluentd                                                                    <span class="conum" data-value="2" />
      image: "fluent/fluentd-kubernetes-daemonset:v1.3.3-debian-elasticsearch-1.3"
      args:
        - "-c"
        - "/etc/fluent.conf"
      env:
        - name: "FLUENTD_CONF"                                                         <span class="conum" data-value="3" />
...
<ul class="colist">
<li data-value="1">The Coherence log level has been set to <code>9</code> to give more verbose logs.</li>
<li data-value="2">The <code>fluentd</code> container has been added to the <code>sideCars</code> list. This will create another container
in the <code>Pod</code> exactly as configured.</li>
<li data-value="3">The <code>FLUENTD_CONF</code> environment variable has been set to the name of the configuration file that Fluentd should use.
...
<p>There are many ways to configure Fluentd, the example above is just one way and is in fact taken from on eof the Operator&#8217;s functional tests.</p>
