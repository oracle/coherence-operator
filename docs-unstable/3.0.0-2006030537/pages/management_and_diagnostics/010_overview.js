<doc-view>

<h2 id="_overview">Overview</h2>
<div class="section">
<p>There are a number of management and diagnostic features available with Oracle Coherence that can be used
in a cluster deployed in Kubernetes.</p>

<v-layout row wrap class="mb-5">
<v-flex xs12>
<v-container fluid grid-list-md class="pa-0">
<v-layout row wrap class="pillars">
<v-flex xs12 sm4 lg3>
<v-card>
<router-link to="/management_and_diagnostics/020_management_over_rest"><div class="card__link-hover"/>
</router-link>
<v-card-title primary class="headline layout justify-center">
<span style="text-align:center">Management Over REST</span>
</v-card-title>
<v-card-text class="caption">
<p></p>
<p>Coherence Management over REST feature</p>
</v-card-text>
</v-card>
</v-flex>
<v-flex xs12 sm4 lg3>
<v-card>
<router-link to="/management_and_diagnostics/030_visualvm"><div class="card__link-hover"/>
</router-link>
<v-card-title primary class="headline layout justify-center">
<span style="text-align:center">VisualVM</span>
</v-card-title>
<v-card-text class="caption">
<p></p>
<p>Coherence VisualVM plugin.</p>
</v-card-text>
</v-card>
</v-flex>
<v-flex xs12 sm4 lg3>
<v-card>
<router-link to="/management_and_diagnostics/040_ssl"><div class="card__link-hover"/>
</router-link>
<v-card-title primary class="headline layout justify-center">
<span style="text-align:center">SSL</span>
</v-card-title>
<v-card-text class="caption">
<p></p>
<p>Enable SSL on the Management over REST endpoint.</p>
</v-card-text>
</v-card>
</v-flex>
</v-layout>
</v-container>
</v-flex>
</v-layout>
</div>
</doc-view>
