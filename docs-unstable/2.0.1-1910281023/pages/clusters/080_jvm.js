...
<li>
<p><router-link to="#jmx" @click.native="this.scrollFix('#jmx')">JMX Management</router-link></p>
...
<h2 id="jmx">JMX Management Configuration</h2>
<div class="section">
<p>Using Java Management Extensions (JMX) can be unreliable in container based environments due to the limitations of the
default RMI transport used by JMX. A more reliable approach is to use an alternative transport and to this end the
Coherence Operator enables JMXMP to be configured and used to expose JMX functionality outside of the containers.</p>

<p>JMXMP is configured using the fields in the <code>jvm.jmxmp</code> section of the configuration.
...
<p>See the <router-link to="/management/040_visualvm">VisualVM Example</router-link> for a detailed example of how to configure a JMX Management
role in a <code>CoherenceCluster</code>.</p>


<h3 id="_configure_jmxmp_for_the_implicit_role">Configure JMXMP for the Implicit Role</h3>
<div class="section">
<p>When creating a <code>CoherenceCluster</code> with a single implicit role the <code>jmxmp</code> is set in the <code>spec.jvm</code>
...
<ul class="colist">
<li data-value="1">JMXMP is enabled for the implicit storage role</li>
</ul>
<markup
lang="yaml"
...
spec:
  jvm:
    jmxmp:
      ...
      port: 7000     <span class="conum" data-value="2" /></markup>

<ul class="colist">
<li data-value="1">JMXMP is enabled for the implicit storage role</li>
<li data-value="2">The JMXMP server will bind to port <code>7000</code> in the container instead of the default <code>9099</code></li>
</ul>
</div>

<h3 id="_setting_container_resource_limits_for_explicit_roles">Setting Container Resource Limits for Explicit Roles</h3>
<div class="section">
<p>When creating a <code>CoherenceCluster</code> with one or more explicit roles the <code>jmxmp</code> can be configured in the <code>jvm</code>
...
spec:
  roles:
    - role: data
      jvm:
        jmxmp:           <span class="conum" data-value="1" />
          enabled: true
          port: 7000
    - role: proxy
      jvm:
        jmxmp:           <span class="conum" data-value="2" />
          enabled: false</markup>

<ul class="colist">
<li data-value="1">The <code>data</code> role has JMXMP enabled and bound to port 7000</li>
<li data-value="2">The <code>proxy</code> role specifically has JMXMP disabled</li>
</ul>
</div>
...
<div class="section">
<p>When creating a <code>CoherenceCluster</code> with one or more explicit roles a default <code>jmxmp</code> values can be configured in
...
spec:
  ...
  roles:
    ...
    - role: proxy
      jvm:
        jmxmp:             <span class="conum" data-value="3" />
          enabled: false
    - role: web            <span class="conum" data-value="4" /></markup>

<ul class="colist">
<li data-value="1">The default settings enable JMXMP support for all roles in the <code>roles</code> list unless overridden</li>
<li data-value="2">The <code>data</code> role will have JMXMP enabled due to the default setting and also adds the <code>port</code> configuration to bind
JMXMP to port <code>7000</code></li>
<li data-value="3">The <code>proxy</code> role explicity overrides the default to disable JMXMP</li>
<li data-value="4">The <code>web</code> role has no specific <code>jmxmp</code> configuration so will use the default that will enable JMXMP and bind the
server to port <code>9099</code></li>
</ul>
<v-divider class="my-5"/>
</div>
</div>
...
<h3 id="_setting_container_resource_limits_for_explicit_roles_2">Setting Container Resource Limits for Explicit Roles</h3>
<div class="section">
...
<h3 id="_setting_container_resource_limits_for_explicit_roles_with_a_default_2">Setting Container Resource Limits for Explicit Roles with a Default</h3>
<div class="section">
