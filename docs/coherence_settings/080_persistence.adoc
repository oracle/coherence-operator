...
distributed caches. Cached data is persisted so that it can be quickly recovered after a catastrophic failure
...
configured. Persistence is configured in the `spec.coherence.persistence` section of the CRD.
...
The persistence mode is set using the `spec.coherence.persistence,mode` field in the CRD. The value of this field is used
to set the `coherence.distributed.persistence-mode` system property in the Coherence JVM.
...
spec:
  coherence:
    persistence:
      mode: active  #<1>
----
<1> Th eexample above sets the persistence mode to `active` which will effectively pass
...
When persistence storage is configured a `VolumeMount` will be added to the Coherence container mounted at `/persistence`,
...
configuring a PVC for a normal Kubernetes Pod and all of the possible options are beyond the scope of this document.
        ...
        persistentVolumeClaim:                   # <2>
          resources:
            requests:
              storage: 50Gi
----
<1> Active persistence data is stored on a normal Volume using a HostPath volume source.
<2> Snapshot data is stored in a 50GB PVC.
...
----
<1> Active persistence data is stored on a normal Volume using a HostPath volume source.
<2> Snapshot data is stored is stored on a normal Volume using a different HostPath volume source.
