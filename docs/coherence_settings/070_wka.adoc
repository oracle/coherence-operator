...
When the Coherence Operator creates reconciles a `CoherenceDeployment` configuration to create a running set of `Pods`
it creates a headless service specifically for the purposes of WKA for that `CoherenceDeployment` with a selector that
...
----
<1> In this yaml the `CoherenceDeployment` has a cluster name of `test-cluster`

The Operator will create a `Service` for the `CoherenceDeployment` using the same name as the deployment
with a `-wka` suffix.
