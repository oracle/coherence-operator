...
The following commands create a `ConfigMap` named `coherence-grafana-dashboards` in the `monitoring` namespace
from the dashboard files in the `dashboards/grafana/` directory, then a label `grafana_dashboard=1` is added to
the `ConfigMap`:
...
Create a values file like the following:
[source,yaml]
