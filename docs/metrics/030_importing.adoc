...
* Default - these are dashboards under the `dashboards/grafana/` directory that are compatible with the metric names produced by the Coherence metrics publisher
* Microprofile - these are dashboards under the `dashboards/grafana-microprofile/` directory that are compatible with the metric names produced by the Coherence MP Metrics module. These are metrics commonly used with Microprofile applications such as Helidon. 
