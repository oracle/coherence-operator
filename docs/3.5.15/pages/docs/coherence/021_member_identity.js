...
<h3 id="down">Using Node Labels Via The Downwards API</h3>
<div class="section">
<p>In Kubernetes 1.34 the enhancement <a id="" title="" target="_blank" href="https://github.com/kubernetes/enhancements/blob/master/keps/sig-node/4742-node-topology-downward-api/README.md">KEP-4742</a>
provides access to Kubernetes topology node labels in Pods using the downwards API.
This means that the topology labels applied to a node that a Pod is scheduled onto are accessible in the Pod as environment variables.</p>

<div class="admonition note">
<p class="admonition-textlabel">Note</p>
<p ><p>In Kubernetes 1.34 this feature is in alpha and has to be enabled using a feature gate.</p>
</p>
</div>
<p>The two topology labels are</p>
...
<thead>
<tr>
<th>topology.kubernetes.io/region</th>
</tr>
</thead>
<tbody>
<tr>
<td class="">topology.kubernetes.io/zone</td>
</tr>
</tbody>
</table>
</div>
<p>These are Kubernetes well-know label names configured by most cloud providers.
For example in an Oracle Kubernetes cluster running in the Phoenix (PHX) region the labels on the node might look like this:</p>
...
cp-3.private.openshiftvcn.oraclevcn.com        Ready    control-plane,master   191d   v1.34.0   phx      PHX-AD-3</markup>

<p>All the nodes of this cluster run in the PHX region, as typically a cluster is in only one region.
The nodes are evenly spread over three availability zones, PHX-AD-1, PHX-AD-2 and PHX-AD-3.</p>

<p>When a Pod is created in K8s the downward API will apply the topology labels from the node as annotations on the Pod.
These annotations can be applied to environment variables in the yaml.</p>

<p>For example, the yaml below sets the <code>KUBERNETES_REGION</code> environment variable to the value of the
<code>topology.kubernetes.io/region</code> label from the Node that the Pod is running on.</p>
...
>  env:
    - name: KUBERNETES_REGION
      valueFrom:
        fieldRef:
          fieldPath: metadata.labels['topology.kubernetes.io/region']</markup>

<p>This can be used in a Coherence resource to set the Coherence site and rack values.</p>
...
spec:
  ...
  env:
    ...
    - name: COHERENCE_RACK
      value: "${KUBERNETES_REGION}-${KUBERNETES_ZONE}"</markup>

<p>In the example above, if the Pod was scheduled onto the node <code>os-cert-cp-1.private.openshiftvcn.oraclevcn.com</code> that is running
in region <code>phx</code> in availability zon <code>PHX-AD-1</code> then the site would be <code>phx-PHX-AD-1</code> and the rack would be <code>phx-PHX-AD-1</code>.</p>

<p>In this case both site and rack are the same. The reason is that when a Coherence cluster is deployed into a single Kubernetes cluster
running in a single site, there is not really a topology node label to represent rack.
We cannot use just the region label for site, because that would mean Coherence only has one site (in this case <code>phx</code>) so
the partitions could not be site safe.
So we combine both the region and zone values together and use them for both site and rack labels.</p>

<div class="admonition note">
<p class="admonition-textlabel">Note</p>
<p ><p>When setting site and rack this way the Pod will not perform a REST call back to the Coherence Operator
to request the site and rack names. This technique is therfore useful in Kubernetes clusters where the
operator is running without Node lookup RBAC permissions.</p>
</p>
</div>
</div>
