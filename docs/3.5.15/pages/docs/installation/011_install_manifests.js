...
<p ><p>As of v3.5.15 of the Operator the manifest yaml also installs the two CRDs that the Operator uses.
...
>kubectl apply -f https://github.com/oracle/coherence-operator/releases/download/v3.5.15/coherence-operator.yaml</markup>
...
<p>The Operator installed with this yaml will not lookup Node labels for Coherence site and rack configurations.
...
See the <router-link :to="{path: '/docs/coherence/021_member_identity', hash: '#down'}">Using Node Labels Via The Downwards API</router-link>
for an alternative way to specify site and rack on Kubernetes 1.34 and above.</p>

<div class="admonition note">
<p class="admonition-textlabel">Note</p>
<p ><p>As of v3.5.15 of the Operator the <code>coherence-operator-restricted.yaml</code> also installs the two CRDs that the Operator uses.
In previous releases the Operator would install the CRDs when it started but this behaviour is disabled by default
when installing with the manifest yaml.</p>
</p>
</div>
...
>kubectl apply -f https://github.com/oracle/coherence-operator/releases/download/v3.5.15/coherence-operator-restricted.yaml</markup>
...
>kubectl apply -f https://github.com/oracle/coherence-operator/releases/download/v3.5.15/coherence.oracle.com_coherence_small.yaml
kubectl apply -f https://github.com/oracle/coherence-operator/releases/download/v3.5.15/coherencejob.oracle.com_coherence_small.yaml</markup>
