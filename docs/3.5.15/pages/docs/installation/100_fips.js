...
Oracle Coherence images used by the operator are FIPS compliant.</p>

<p>The Oracle Coherence team does not currently publish FIPS compliant Coherence images.
Customers must build their own FIPS complaint Java and Coherence images, which the operator will then manage.</p>

<p>Coherence is FIPS <em>compatible</em> so a correctly configured application running in an image that has a FIPS
compliant JDK and FIPS compliant base O/S will be FIPS complaint. To be fully FIPS compliant these images must then
be run in a container runtime on a FIPS compliant host.</p>
</p>
</div>
...
<div class="section">
<p>If <router-link to="/docs/installation/013_install_kustomize">installing the operator using Kustomize</router-link> (or using <code>kubectl -k</code>)
the Coherence manifest yaml files contain a FIPS overlay that adds the <code>GODEBUG</code> environment variable
to the Operator container.</p>

<p>The following command will generate a yaml manifest that installs the operator with FIPS enabled:</p>

<markup
lang="bash"

>kustomize build manifests/overlays/fips</markup>
