...
<code>container-registry.oracle.com/middleware/coherence-operator:3.5.15</code>.
...
with the same image name <code>coherence-operator</code> and tag <code>3.5.15</code> as the default image,
then just the <code>image.registry</code> needs to be specified.</p>

<p>In the example below, the image used to run the Operator will be <code>foo.com/coherence-operator:3.5.15</code>.</p>
...
<h3 id="helm-rest-service-ip-families">Configure the Operator REST Service IP Families</h3>
<div class="section">
<p>The <code>restService.ipFamilyPolicy</code> and <code>restService.ipFamilies</code> values configure the corresponding
Kubernetes fields on the <code>coherence-operator-rest</code> Service. Both values are unset by default, so
Kubernetes applies the cluster&#8217;s normal Service defaults. Either value may be set independently.</p>

<p>Valid values for <code>restService.ipFamilyPolicy</code> are <code>SingleStack</code>, <code>PreferDualStack</code>, and
<code>RequireDualStack</code>. The <code>restService.ipFamilies</code> value is an ordered list containing at most two
distinct entries, each of which must be <code>IPv4</code> or <code>IPv6</code>.</p>

<p>The following values configure an IPv4-only REST Service:</p>
...
<p>For an IPv6-only REST Service, use:</p>
...
<p>For a dual-stack REST Service with IPv4 as the primary family, use:</p>
...
<p>The family order can be reversed to make IPv6 primary:</p>
...
<p>Helm validates the value types, policy and family names, family count, and uniqueness. Kubernetes
...
<p>Separately from these Service settings, the Operator REST listener binds to the unspecified wildcard
address by default. This allows IPv4 and IPv6 connections when the platform supports dual-stack
sockets, and falls back to IPv4 when IPv6 is unavailable. The <code>--rest-host</code> flag remains available for
an explicit bind address; use <code>0.0.0.0</code> for an IPv4-only wildcard, <code>::</code> for an IPv6 wildcard, or
specify an individual IPv4 or IPv6 address. IPv6 addresses may optionally be enclosed in square
brackets. Changing the listener address does not change the Service&#8217;s IP-family allocation.</p>
...
<h3 id="helm-upgrade-350">Upgrading From pre-3.5.15 Versions</h3>
<div class="section">
<p>Before version 3.5.15 of the Coherence operator, the operator used to install the CRDs
when it started. In 3.5.15 this behaviour was changed and the operator no longer installs
the CRDs, these must be installed along with the operator. The 3.5.15 and above Helm chart
includes the CRDs.</p>

<p>This causes an issue when performing a Helm upgrade from a pre-3.5.15 version because Helm
