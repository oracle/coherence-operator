...
<h3 id="start-timeout">The Operator fails to start with a timeout error</h3>
<div class="section">
<p>One of the first things the operator does on start-up os to log the Kubernetes server version.
...
request will time out with an error like the one below</p>
...
Error: unable to get kubernetes server version: Get "https://10.96.0.1:443/version?timeout=32s": dial tcp 10.96.0.1:443: i/o timeout</markup>

<p>The error message in the operator&#8217;s logs will include the host name and port that the operator attempted to use,
so this can be used to then ensure egress is allowed to the correct host and port.</p>

</div>

<h3 id="no-operator">I Uninstalled the Operator and Cannot Delete the Coherence Clusters</h3>
<div class="section">
<p>The <code>Coherence</code> resources managed by the Operator are marked in k8s as being owned by the Operator, and have finalizers to stop them being deleted. In normal operation the Operator will remove the finalizer when it deletes a <code>Coherence</code> cluster.</p>
...
<p>If you have overridden the configuration for the readiness or liveness probes for the Coherence cluster with custom endpoints,
then you need to debug your own code.</p>
