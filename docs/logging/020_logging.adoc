...
If the `jvm.loggingConfig` field is set the value is used to set the `java.util.logging.config.file`
system property.
If `jvm.loggingConfig` is set the operator will also set the `coherence.log` system property to `jdk`
(i.e. `-Dcoherence.log=jdk`), this tells Coherence to use the JDK logger (i.e. Java util logging).
...
----
The `jvm.loggingConfig` field has been set to use the config file provided by the operator.
