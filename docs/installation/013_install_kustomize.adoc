...
https://github.com/oracle/coherence-operator/releases/download/v3.5.15/coherence-operator-manifests.tar.gz[3.5.15 Manifests]
...
[source,bash]
----
cd ./manager && kustomize edit set image controller=myregistry/coherence-operator:3.5.15
----
...
[source,bash]
----
cd ./manager && kustomize edit add configmap env-vars --from-literal OPERATOR_IMAGE=myregistry/coherence-operator:3.5.15
----
