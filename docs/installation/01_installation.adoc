...
As the CRD version is fixed at `v1` there is no guarantee which CRD version has actually installed, which could
...
beginning of the file. The CRDs *_must be installed before the Operator is installed_*, as described
...
[NOTE]
====
As of v3.4.3 of the Operator the `coherence-operator-restricted.yaml` also installs the two CRDs that the
Operator uses.
In previous releases the Operator would install the CRDs when it started but this behaviour has been removed.
====
...
By default, the Operator install manifests contains both CRDs, `Coherence` and `CoherenceJob`.
If support for `CoherenceJob` is not required then it can be excluded from being ued by setting the
Operator command line parameter `--enabled-jobs` to `false`.

When installing with Helm, the `allowCoherenceJobs` value can be set to `false` to disable support
for `CoherenceJob` resources (the default value is `true`).
...
[NOTE]
====
As of v3.4.3 of the Operator the manifest yaml also installs the two CRDs that the Operator uses.
In previous releases the Operator would install the CRDs when it started but this behaviour has been removed.
====
