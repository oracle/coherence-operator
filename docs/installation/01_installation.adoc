...
The Coherence Operator uses two images, the Operator itself and a utilities image that runs as an init-container in the Coherence cluster Pods.

* `{operator-image}` - The Operator image.
* `{operator-utils-image}` - The Operator Utilities image.
...
[source,bash]
----
kubectl apply -f https://github.com/oracle/coherence-operator/releases/download/v3.2.7/coherence-operator.yaml
----
...
https://github.com/oracle/coherence-operator/releases/download/v3.2.7/coherence-operator-manifests.tar.gz[3.2.7 Manifests]
...
[source,bash]
----
cd ./manager && kustomize edit set image controller=myregistry/coherence-operator:3.2.7
----
...
[source,bash]
----
cd ./manager && kustomize edit add configmap env-vars --from-literal UTILS_IMAGE=myregistry/coherence-operator:3.2.7-utils
----
...
coherence-operator.oracle.github.com  Oracle Coherence Operator  A Kubernetes operator for managing Oracle Coherence clusters  3.2.7
----
...
coherence  coherence-operator.oracle.github.com  3.2.7            Reconcile succeeded
----
