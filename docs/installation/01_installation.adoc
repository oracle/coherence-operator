...
[source,bash]
----
kubectl apply -f https://github.com/oracle/coherence-operator/releases/download/v3.2.6/coherence-operator.yaml
----
...
https://github.com/oracle/coherence-operator/releases/download/v3.2.6/coherence-operator-manifests.tar.gz[3.2.6 Manifests]
...
[source,bash]
----
cd ./manager && kustomize edit set image controller=myregistry/coherence-operator:3.2.6
----
...
[source,bash]
----
cd ./manager && kustomize edit add configmap env-vars --from-literal UTILS_IMAGE=myregistry/coherence-operator:3.2.6-utils
----
