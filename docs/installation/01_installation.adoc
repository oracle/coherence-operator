...
* Access to a Kubernetes v1.13.0+ cluster. The Operator test pipeline runs on Kubernetes tested on v1.13 upto v1.20
...
If you want the default Coherence Operator installation then the simplest solution is

[source,bash]
----
kubectl apply -f https://github.com/oracle/coherence-operator/releases/download/v3.1.4/coherence-operator.yaml
----

This will create a namespace called `coherence` and install the Operator into it along with all the required `ClusterRole` and `RoleBinding` resources.

The `coherence` namespace can be changed by editing the yaml file.
...
https://github.com/oracle/coherence-operator/releases/download/v3.1.3/coherence-operator-manifests.tar.gz[3.1.4 Manifests]
...
[source,bash]
----
cd ./manager && kustomize edit set image controller=container-registry.oracle.com/middleware/coherence-operator:3.1.4
----
...
[source,bash]
----
cd ./manager && kustomize edit add configmap env-vars --from-literal UTILS_IMAGE=container-registry.oracle.com/middleware/coherence-operator:3.1.4-utils
----
