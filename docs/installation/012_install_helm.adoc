...
[#helm-upgrade-350]
=== Upgrading From pre-3.5.14 Versions

Before version 3.5.14 of the Coherence operator, the operator used to install the CRDs
when it started. In 3.5.14 this behaviour was changed and the operator no longer installs
the CRDs, these must be installed along with the operator. The 3.5.14 and above Helm chart
includes the CRDs.

This causes an issue when performing a Helm upgrade from a pre-3.5.14 version because Helm
