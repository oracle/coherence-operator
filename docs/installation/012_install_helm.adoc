...
[#helm-upgrade-350]
=== Upgrading From pre-3.5.1 Versions

Before version 3.5.1 of the Coherence operator, the operator used to install the CRDs
...
This causes an issue when performing a Helm upgrade from a pre-3.5.1 version because Helm
