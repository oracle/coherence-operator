...
[#helm-upgrade-350]
=== Upgrading From pre-3.5.13 Versions

Before version 3.5.13 of the Coherence operator, the operator used to install the CRDs
when it started. In 3.5.13 this behaviour was changed and the operator no longer installs
the CRDs, these must be installed along with the operator. The 3.5.13 and above Helm chart
includes the CRDs.

This causes an issue when performing a Helm upgrade from a pre-3.5.13 version because Helm
