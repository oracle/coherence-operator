...
[NOTE]
====
As of v3.5.12 of the Operator the manifest yaml also installs the two CRDs that the Operator uses.
...
[source,bash]
----
kubectl apply -f https://github.com/oracle/coherence-operator/releases/download/v3.5.12/coherence-operator.yaml
----
...
[NOTE]
====
As of v3.5.12 of the Operator the `coherence-operator-restricted.yaml` also installs the two CRDs that the Operator uses.
In previous releases the Operator would install the CRDs when it started but this behaviour is disabled by default
when installing with the manifest yaml.
====
...
[source,bash]
----
kubectl apply -f https://github.com/oracle/coherence-operator/releases/download/v3.5.12/coherence-operator-restricted.yaml
----
[IMPORTANT]
====
In some restricted environments, a Kubernetes user might not have RBAC permissions to install CRDs.
...
[source,bash]
----
kubectl apply -f https://github.com/oracle/coherence-operator/releases/download/v3.5.12/coherence.oracle.com_coherence_small.yaml
kubectl apply -f https://github.com/oracle/coherence-operator/releases/download/v3.5.12/coherencejob.oracle.com_coherence_small.yaml
----
