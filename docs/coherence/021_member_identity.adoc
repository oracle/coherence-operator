...
* The rack name is taken from the `topology.kubernetes.io/zone` label on the Node that the Pod has been scheduled onto.
...
will be tried.
If neither of these labels are set then the site will be unset.
