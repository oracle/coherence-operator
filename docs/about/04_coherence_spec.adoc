...
m| runAsJob | RunAsJob specifies that the cluster should be created as a Job instead of a StatefulSet. m| &#42;<<CoherenceJob,CoherenceJob>> | false
|===
...
m| activeDeadlineSeconds | Specifies the duration in seconds relative to the startTime that the job may be continuously active before the system tries to terminate it; value must be positive integer. If a Job is suspended (at creation or through an update), this timer will effectively be stopped and reset when the Job is resumed again. m| &#42;int64 | false
