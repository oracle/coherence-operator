...
The Coherence container in the `Pods` in a `CoherenceDeployments` runs a Java application and as such requires a classpath
...
classpath. In this case the `CoherenceDeployment` spec has a field to specify that the JIB classpath should not be used.
...
to specify additional elements to be added to the classpath. The `jvm.classpath` field in the `CoherenceDeployment` spec
